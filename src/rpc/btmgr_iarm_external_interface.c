...
BTRMGR_RegisterForCallbacks (
    ...
    IARM_Bus_RegisterEventHandler(IARM_BUS_BTRMGR_NAME, BTRMGR_IARM_EVENT_DEVICE_UNSUPPORTED, btrMgrdeviceCallback);
...
BTRMGR_UnRegisterFromCallbacks (
    ...
    IARM_Bus_UnRegisterEventHandler(IARM_BUS_BTRMGR_NAME, BTRMGR_IARM_EVENT_DEVICE_UNSUPPORTED);
...
BTRMGR_Result_t BTRMGR_LE_StartAdvertisement(unsigned char aui8AdapterIdx, BTRMGR_LeCustomAdvertisement_t *pstBTMGR_LeCustomAdvt)
{
    ...
    MEMCPY_S(&lstAdvtInfo.m_CustAdvt,sizeof(lstAdvtInfo.m_CustAdvt), pstBTMGR_LeCustomAdvt, sizeof(BTRMGR_LeCustomAdvertisement_t));
...
BTRMGR_SetLimitBeaconDetection (
    ...
    MEMSET_S(&beaconDetection, sizeof(BTRMGR_IARMBeaconDetection_t), 0, sizeof(BTRMGR_IARMBeaconDetection_t));
    beaconDetection.m_adapterIndex = index_of_adapter;
...
BTRMGR_GetAdapterName (
    ...
    adapterSetting.m_adapterIndex = index_of_adapter;
    MEMSET_S(adapterSetting.m_name, sizeof (adapterSetting.m_name), '\0', sizeof (adapterSetting.m_name));
...
BTRMGR_GetDiscoveryStatus (
    ...
    MEMSET_S(&discoveryStatus, sizeof(BTRMGR_IARMDiscoveryStatus_t), 0, sizeof(BTRMGR_IARMDiscoveryStatus_t));
...
BTRMGR_GetDiscoveredDevices (
    ...
    if (IARM_RESULT_SUCCESS == retCode) {
        MEMCPY_S(pDiscoveredDevices,sizeof(BTRMGR_DiscoveredDevicesList_t), &discoveredDevices.m_devices, sizeof(BTRMGR_DiscoveredDevicesList_t));
        BTRMGRLOG_INFO ("Success\n");
    }
    else {
        rc = BTRMGR_RESULT_GENERIC_FAILURE;
...
BTRMGR_PairDevice (
    ...
    MEMSET_S(&newDevice, sizeof(newDevice), 0, sizeof(newDevice));
...
BTRMGR_UnpairDevice (
    ...
    MEMSET_S(&removeDevice, sizeof(removeDevice), 0, sizeof(removeDevice));
...
BTRMGR_GetPairedDevices (
    ...
    if (IARM_RESULT_SUCCESS == retCode) {
        MEMCPY_S(pPairedDevices,sizeof(BTRMGR_PairedDevicesList_t), &pairedDevices.m_devices, sizeof(BTRMGR_PairedDevicesList_t));
        BTRMGRLOG_INFO ("Success\n");
    }
    else {
        rc = BTRMGR_RESULT_GENERIC_FAILURE;
...
BTRMGR_GetConnectedDevices (
    ...
    if (IARM_RESULT_SUCCESS == retCode) {
        MEMCPY_S(pConnectedDevices,sizeof(BTRMGR_ConnectedDevicesList_t), &connectedDevices.m_devices, sizeof(BTRMGR_ConnectedDevicesList_t));
        BTRMGRLOG_INFO ("Success\n");
    }
    else {
        rc = BTRMGR_RESULT_GENERIC_FAILURE;
...
BTRMGR_GetDeviceProperties (
    ...
    if (IARM_RESULT_SUCCESS == retCode) {
        MEMCPY_S(pDeviceProperty,sizeof(BTRMGR_DevicesProperty_t), &deviceProperty.m_deviceProperty, sizeof(BTRMGR_DevicesProperty_t));
        BTRMGRLOG_INFO ("Success\n");
    }
    else {
        rc = BTRMGR_RESULT_GENERIC_FAILURE;
...
BTRMGR_GetDeviceBatteryLevel (
    ...
    BTRMGR_Result_t rc = BTRMGR_RESULT_SUCCESS;
    IARM_Result_t retCode = IARM_RESULT_SUCCESS;
    BTRMGR_IARMDeviceBatteryLevel_t deviceBatteryLevel;

    if ((BTRMGR_ADAPTER_COUNT_MAX < index_of_adapter) || (0 == handle) || (NULL == pDeviceBatteryLevel)) {
        rc = BTRMGR_RESULT_INVALID_INPUT;
        BTRMGRLOG_ERROR ("Input is invalid\n");
        return rc;
    }
    ...
    deviceBatteryLevel.m_adapterIndex = index_of_adapter;
    deviceBatteryLevel.m_deviceHandle = handle;

    retCode = IARM_Bus_Call_with_IPCTimeout(IARM_BUS_BTRMGR_NAME, BTRMGR_IARM_METHOD_GET_BATTERY_LEVEL, (void *)&deviceBatteryLevel, sizeof(deviceBatteryLevel), BTRMGR_IARM_METHOD_CALL_TIMEOUT_DEFAULT_MS);
    if (IARM_RESULT_SUCCESS == retCode) {
        *pDeviceBatteryLevel = deviceBatteryLevel.deviceBatteryLevel;
        BTRMGRLOG_INFO ("Success\n");
    }
    else {
        rc = BTRMGR_RESULT_GENERIC_FAILURE;
...
BTRMGR_SetEventResponse (
    ...
    MEMCPY_S(&lstBtrMgrIArmEvtResp.m_stBTRMgrEvtRsp,sizeof(lstBtrMgrIArmEvtResp.m_stBTRMgrEvtRsp), apstBTRMgrEvtRsp, sizeof(BTRMGR_EventResponse_t));
...
BTRMGR_MediaControl (
    ...
    MEMSET_S(&mediaProperty, sizeof(BTRMGR_IARMMediaProperty_t), 0, sizeof(BTRMGR_IARMMediaProperty_t));
...
BTRMGR_GetDataPathAndConfigurationForStreamOut(
    ...
    BTRMGR_Result_t rc = BTRMGR_RESULT_SUCCESS;
    IARM_Result_t retCode = IARM_RESULT_SUCCESS;
    BTRMGR_IARMDeviceStreamingConfig_t devStreamConf;

    if (BTRMGR_ADAPTER_COUNT_MAX < aui8AdapterIdx 
        || NULL == pstBtrMgrDevStreamInfo 
        || NULL == pi32Delay || pi32Writemtu == NULL 
        || pi32Readmtu == NULL || pi32dataPath == NULL ) 
    {
        rc = BTRMGR_RESULT_INVALID_INPUT;
        BTRMGRLOG_ERROR ("Input is invalid\n");
        return rc;
    ...
    devStreamConf.m_adapterIndex = aui8AdapterIdx;
    devStreamConf.m_deviceHandle = ahBTRMgrDevHdl;
    retCode = IARM_Bus_Call_with_IPCTimeout(IARM_BUS_BTRMGR_NAME, BTRMGR_IARM_METHOD_GET_DEVICE_DATA_PATH_CONF_INFO, (void*)&devStreamConf, sizeof(BTRMGR_IARMDeviceStreamingConfig_t), BTRMGR_IARM_METHOD_CALL_TIMEOUT_DEFAULT_MS);
    
    if (retCode == IARM_RESULT_SUCCESS)
    {
        *pi32dataPath = devStreamConf.m_pi32dataPath;
        *pi32Delay = devStreamConf.m_pi32Delay;
        *pi32Writemtu = devStreamConf.m_pi32Writemtu;
        *pi32Readmtu = devStreamConf.m_pi32Readmtu;
        MEMCPY_S(pstBtrMgrDevStreamInfo, sizeof(BTRMGR_MediaStreamInfo_t), &devStreamConf.m_pstCodecInformation, sizeof(BTRMGR_MediaStreamInfo_t));
        return BTRMGR_RESULT_SUCCESS;
    }
    else
        return BTRMGR_RESULT_GENERIC_FAILURE;
...
BTRMGR_ReleaseDataPathForStreamOut(
    ...
    BTRMGR_Result_t rc = BTRMGR_RESULT_SUCCESS;
    IARM_Result_t retCode = IARM_RESULT_SUCCESS;
    BTRMGR_IARMDeviceStreamingRelease_t devRelease;
    ...
    devRelease.m_adapterIndex = aui8AdapterIdx;
    devRelease.m_deviceHandle = ahBTRMgrDevHdl;
    retCode = IARM_Bus_Call_with_IPCTimeout(IARM_BUS_BTRMGR_NAME, BTRMGR_IARM_METHOD_RELEASE_DATA_PATH, (void*)&devRelease, sizeof(BTRMGR_IARMDeviceStreamingRelease_t), BTRMGR_IARM_METHOD_CALL_TIMEOUT_DEFAULT_MS);
...
)
{
    BTRMGR_Result_t rc = BTRMGR_RESULT_SUCCESS;
    IARM_Result_t retCode = IARM_RESULT_SUCCESS;
    BTRMGR_IARMDeviceFileStreamingConfig_t fileStreamingOutConf;

    if (BTRMGR_ADAPTER_COUNT_MAX < aui8AdapterIdx || !pstMediaAudioOutInfo || !pstMediaAudioInInfo || !pcAudioInputFilePath) 
    {
        rc = BTRMGR_RESULT_INVALID_INPUT;
        BTRMGRLOG_ERROR ("Input is invalid\n");
        return rc;
    }
    fileStreamingOutConf.m_adapterIndex = aui8AdapterIdx;
    fileStreamingOutConf.m_deviceHandle = ahBTRMgrDevHdl;
    fileStreamingOutConf.m_outputFd = i32OutFd;
    fileStreamingOutConf.m_outMtu = outMTUSize;
    fileStreamingOutConf.m_outDevDelay = outDevDelay;
    MEMCPY_S(&fileStreamingOutConf.m_audioConfigOut, sizeof(BTRMGR_MediaStreamInfo_t), pstMediaAudioOutInfo, sizeof(BTRMGR_MediaStreamInfo_t));
    MEMCPY_S(&fileStreamingOutConf.m_audioConfigIn, sizeof(BTRMGR_MediaStreamInfo_t), pstMediaAudioInInfo, sizeof(BTRMGR_MediaStreamInfo_t));
    strncpy(fileStreamingOutConf.m_filePath, pcAudioInputFilePath, BTRMGR_MAX_STR_LEN);

    retCode = IARM_Bus_Call_with_IPCTimeout(IARM_BUS_BTRMGR_NAME, BTRMGR_IARM_METHOD_START_STREAMING_AUDIO_FROM_FILE, (void*)&fileStreamingOutConf, sizeof(BTRMGR_IARMDeviceFileStreamingConfig_t), BTRMGR_IARM_METHOD_CALL_TIMEOUT_DEFAULT_MS);
    if (retCode == IARM_RESULT_SUCCESS)
        return BTRMGR_RESULT_SUCCESS;
    else
        return BTRMGR_RESULT_GENERIC_FAILURE;
...
)
{
    ...
    retCode = IARM_Bus_Call_with_IPCTimeout(IARM_BUS_BTRMGR_NAME, BTRMGR_IARM_METHOD_STOP_STREAMING_AUDIO_FROM_FILE, NULL, 0, BTRMGR_IARM_METHOD_CALL_TIMEOUT_DEFAULT_MS);
    if (retCode == IARM_RESULT_SUCCESS)
        return BTRMGR_RESULT_SUCCESS;
    else
        return BTRMGR_RESULT_GENERIC_FAILURE;
...
BTRMGR_GetDeviceVolumeMute (
    ...
    MEMSET_S(&devvolmut, sizeof(BTRMGR_IARMDeviceVolumeMute_t), 0, sizeof(BTRMGR_IARMDeviceVolumeMute_t));
...
BTRMGR_GetDeviceDelay (
    ...
    BTRMgrDeviceHandle           ahBTRMgrDevHdl,
    BTRMGR_DeviceOperationType_t deviceOpType,
    unsigned int *puiDelay,
    unsigned int *puiMsInQueue
) {

    BTRMGR_Result_t rc = BTRMGR_RESULT_SUCCESS;
    IARM_Result_t retCode = IARM_RESULT_SUCCESS;
    BTRMGR_IARMDeviceDelay_t devdelay;

    if ((BTRMGR_ADAPTER_COUNT_MAX < aui8AdapterIdx) || (NULL == puiDelay) || (NULL == puiMsInQueue)) {
        rc = BTRMGR_RESULT_INVALID_INPUT;
        BTRMGRLOG_ERROR ("Input is invalid\n");
        return rc;
    }

    MEMSET_S(&devdelay, sizeof(BTRMGR_IARMDeviceDelay_t), 0, sizeof(BTRMGR_IARMDeviceDelay_t));
    devdelay.m_adapterIndex    = aui8AdapterIdx;
    devdelay.m_deviceHandle    = ahBTRMgrDevHdl;
    devdelay.m_deviceOpType    = deviceOpType;

    retCode = IARM_Bus_Call_with_IPCTimeout(IARM_BUS_BTRMGR_NAME, BTRMGR_IARM_METHOD_GET_DEVICE_DELAY_INFO, (void*)&devdelay, sizeof(BTRMGR_IARMMediaProperty_t), BTRMGR_IARM_METHOD_CALL_TIMEOUT_DEFAULT_MS);

    if (IARM_RESULT_SUCCESS == retCode) {
        *puiDelay =  devdelay.m_delay;
        *puiMsInQueue   =  devdelay.m_msinqueue;
        BTRMGRLOG_INFO ("Success\n");
    }
    else {
        rc = BTRMGR_RESULT_GENERIC_FAILURE;
...
BTRMGR_SetDeviceDelay (
    ...
    devdelay.m_deviceHandle    = ahBTRMgrDevHdl;
    devdelay.m_deviceOpType    = deviceOpType;
    devdelay.m_delay          = uiDelay;

    retCode = IARM_Bus_Call_with_IPCTimeout(IARM_BUS_BTRMGR_NAME, BTRMGR_IARM_METHOD_SET_DEVICE_DELAY_INFO, (void*)&devdelay, sizeof(BTRMGR_IARMMediaProperty_t), BTRMGR_IARM_METHOD_CALL_TIMEOUT_DEFAULT_MS);
...
BTRMGR_GetMediaTrackInfo (
    ...
    if (IARM_RESULT_SUCCESS == retCode) {
        MEMCPY_S(mediaTrackInfo,sizeof(BTRMGR_MediaTrackInfo_t), &mediaProperty.m_mediaTrackInfo, sizeof(BTRMGR_MediaTrackInfo_t));
        BTRMGRLOG_INFO ("Success\n");
    }
    else {
        rc = BTRMGR_RESULT_GENERIC_FAILURE;
...
BTRMGR_GetMediaCurrentPosition (
    ...
    if (IARM_RESULT_SUCCESS == retCode) {
        MEMCPY_S(mediaPositionInfo,sizeof(BTRMGR_MediaPositionInfo_t), &mediaProperty.m_mediaPositionInfo, sizeof(BTRMGR_MediaPositionInfo_t));
        BTRMGRLOG_INFO ("Success\n");
    }
    else {
        rc = BTRMGR_RESULT_GENERIC_FAILURE;
...
BTRMGR_SetMediaElementActive (
    ...
    MEMSET_S(&mediaElementList, sizeof(BTRMGR_IARMMediaElementListInfo_t), 0, sizeof(BTRMGR_IARMMediaElementListInfo_t));
...
BTRMGR_GetMediaElementList (
    ...
    if (IARM_RESULT_SUCCESS == retCode) {
        BTRMGRLOG_INFO ("Success\n");
        MEMCPY_S(mediaElementListInfo,sizeof(BTRMGR_MediaElementListInfo_t), &mediaElementList.m_mediaTrackListInfo, sizeof(BTRMGR_MediaElementListInfo_t));
    }
    else {
        rc = BTRMGR_RESULT_GENERIC_FAILURE;
        BTRMGRLOG_ERROR ("Failed; RetCode = %d\n", retCode);
...
BTRMGR_GetLeProperty (
    ...
    MEMSET_S(&leProperty, sizeof(BTRMGR_IARMLeProperty_t), 0, sizeof(BTRMGR_IARMLeProperty_t));
    ...
    if (IARM_RESULT_SUCCESS == retCode) {
        ...
        case BTRMGR_LE_PROP_UUID:
            MEMCPY_S(apBtrPropValue,sizeof(leProperty.m_uuidList), &leProperty.m_uuidList, sizeof(leProperty.m_uuidList));
            break;
        case BTRMGR_LE_PROP_DEVICE:
            MEMCPY_S(apBtrPropValue, sizeof(leProperty.m_devicePath), &leProperty.m_devicePath, sizeof(leProperty.m_devicePath)-1);
            break;
        case BTRMGR_LE_PROP_SERVICE:
            MEMCPY_S(apBtrPropValue, sizeof(leProperty.m_servicePath), &leProperty.m_servicePath, sizeof(leProperty.m_servicePath)-1);
            break;
        case BTRMGR_LE_PROP_CHAR:
            MEMCPY_S(apBtrPropValue, sizeof(leProperty.m_characteristicPath), &leProperty.m_characteristicPath, sizeof(leProperty.m_characteristicPath)-1);
            break;
        case BTRMGR_LE_PROP_VALUE:
            MEMCPY_S(apBtrPropValue, sizeof(leProperty.m_value), &leProperty.m_value,  sizeof(leProperty.m_value)-1);
            break;
        ...
        case BTRMGR_LE_PROP_FLAGS:
            MEMCPY_S(apBtrPropValue, sizeof(leProperty.m_flags), &leProperty.m_flags, sizeof(leProperty.m_flags));
        default:
            break;
        }
        BTRMGRLOG_INFO ("Success\n");
...
BTRMGR_PerformLeOp (
    ...
    MEMSET_S(&leOp, sizeof(BTRMGR_IARMLeOp_t), 0, sizeof(BTRMGR_IARMLeOp_t));
    ...
    if (IARM_RESULT_SUCCESS == retCode) {
        if (BTRMGR_LE_OP_READ_VALUE == aLeOpType) {
            MEMCPY_S(rOpResult,BTRMGR_MAX_STR_LEN, leOp.m_opRes, BTRMGR_MAX_STR_LEN - 1);
        }
        BTRMGRLOG_INFO ("Success\n");
    }
    else {
...
BTRMGR_SetAudioInServiceState (
    ...
    MEMSET_S(&audioInServiceState, sizeof(BTRMGR_IARMAudioInServiceState_t), 0, sizeof(BTRMGR_IARMAudioInServiceState_t));
...
BTRMGR_SetHidGamePadServiceState (
    ...
    MEMSET_S(&hidGamePadServiceState, sizeof(BTRMGR_IARMHidGamePadServiceState_t), 0, sizeof(BTRMGR_IARMHidGamePadServiceState_t));
...
BTRMGR_SetBtmgrDebugModeState (
    ...
    if (IARM_RESULT_SUCCESS == IARM_Bus_Call_with_IPCTimeout(IARM_BUS_BTRMGR_NAME, BTRMGR_IARM_METHOD_SET_BTMGR_DEBUG_MODE_STATE, (void*)&aui8ServiceState, sizeof(unsigned char), BTRMGR_IARM_METHOD_CALL_TIMEOUT_DEFAULT_MS)) {
        BTRMGRLOG_INFO ("Success\n");
        return BTRMGR_RESULT_SUCCESS;
    }
    else {
        BTRMGRLOG_ERROR ("Failed\n");
        return BTRMGR_RESULT_GENERIC_FAILURE;
    }
...
BTRMGR_SysDiagInfo(
    ...
    MEMSET_S(&lDiagInfo, sizeof(BTRMGR_IARMDiagInfo_t), 0, sizeof(BTRMGR_IARMDiagInfo_t));
...
BTRMGR_ConnectToWifi(
    ...
    MEMSET_S(&lWifiInfo, sizeof(BTRMGR_IARMWifiConnectInfo_t), 0, sizeof(BTRMGR_IARMWifiConnectInfo_t));
...
btrMgrdeviceCallback (
    ...
    if (NULL == pData) {
        ...
        if ((BTRMGR_IARM_EVENT_DEVICE_OUT_OF_RANGE                  == eventId) ||
            ...
            (BTRMGR_IARM_EVENT_DEVICE_UNSUPPORTED                   == eventId) ||
            ...
            MEMCPY_S(&newEvent,sizeof(BTRMGR_EventMessage_t), pReceivedEvent, sizeof(BTRMGR_EventMessage_t));
