...
BTRMgr_SO_SetVolume (
    ...
    if ((leBtrMgrSoGstRet = BTRMgr_SO_GstSetVolume(pstBtrMgrSoHdl->hBTRMgrSoGstHdl, ui8Volume)) != eBTRMgrSOGstSuccess) {
         ...
         BTRMGRLOG_TRACE("Volume set %d success \n", ui8Volume);
    }
...
BTRMgr_SO_GetVolume (
    ...
    if ((leBtrMgrSoGstRet = BTRMgr_SO_GstGetVolume(pstBtrMgrSoHdl->hBTRMgrSoGstHdl, &ui8GstVolume)) != eBTRMgrSOGstSuccess) {
         ...
         *ui8Volume = ui8GstVolume;
         BTRMGRLOG_INFO("Volume get %d success \n", ui8GstVolume);
    }
...
BTRMgr_SO_SetDelay (
    ...
    if (!hBTRMgrSoHdl)
    {
        BTRMGRLOG_ERROR("invalid parameter: hBTRMgrSoHdl");
        return eBTRMgrFailInArg;
    }

    if ((leBtrMgrSoGstRet = BTMgr_SO_GstSetDelay(pstBtrMgrSoHdl->hBTRMgrSoGstHdl, ui16Delay)) != eBTRMgrSOGstSuccess) {
         BTRMGRLOG_ERROR("Return Status = %d - Failed to set delay\n", leBtrMgrSoGstRet);
         leBtrMgrSoRet  = eBTRMgrFailure;
    } else {
         //pstBtrMgrSoHdl->lstBtrMgrSoStatus.ui8Volume = ui16Delay;
         BTRMGRLOG_INFO("Delay set %d success \n", ui16Delay);
    }
...
BTRMgr_SO_GetDelay (
    tBTRMgrSoHdl            hBTRMgrSoHdl,
    unsigned int*               ui16Delay,
    unsigned int*               ui16MsInQueue
) {
    eBTRMgrRet      leBtrMgrSoRet  = eBTRMgrSuccess;
    stBTRMgrSOHdl*  pstBtrMgrSoHdl = (stBTRMgrSOHdl*)hBTRMgrSoHdl;
    eBTRMgrSOGstRet leBtrMgrSoGstRet = eBTRMgrSOGstSuccess;
    unsigned int  lui16GstDelay;
    unsigned int  lui16MsInQueue;

    if(!ui16Delay || !pstBtrMgrSoHdl || !ui16MsInQueue) {
       leBtrMgrSoRet  = eBTRMgrFailure;
       return leBtrMgrSoRet;
    }

    if ((leBtrMgrSoGstRet = BTMgr_SO_GstGetDelay(pstBtrMgrSoHdl->hBTRMgrSoGstHdl, &lui16GstDelay, &lui16MsInQueue )) != eBTRMgrSOGstSuccess) {
         BTRMGRLOG_ERROR("Return Status = %d - Failed to get delay\n", leBtrMgrSoGstRet);
         leBtrMgrSoRet  = eBTRMgrFailure;
    } else {
         *ui16Delay = lui16GstDelay;
         *ui16MsInQueue = lui16MsInQueue;
         BTRMGRLOG_INFO("Delay get %d success (ms in queue: %d) \n", lui16GstDelay, lui16MsInQueue);
    }
...
eBTRMgrRet
BTRMgr_SO_UpdateDelayDynamically (
    tBTRMgrSoHdl            hBTRMgrSoHdl,
    unsigned int            ui16DeviceDelay
) {
    unsigned int newDelay;
    stBTRMgrSOHdl*  pstBtrMgrSoHdl = (stBTRMgrSOHdl*)hBTRMgrSoHdl;
    //calculate new delay
    BTRMgr_SO_GstCalculateDelayNeeded(pstBtrMgrSoHdl->hBTRMgrSoGstHdl, ui16DeviceDelay, &newDelay);
    //set new delay
    BTMgr_SO_GstSetDelay(pstBtrMgrSoHdl->hBTRMgrSoGstHdl, newDelay);
...
BTRMgr_SO_GetMute (
    ...
    if ((leBtrMgrSoGstRet = BTRMgr_SO_GstGetMute(pstBtrMgrSoHdl->hBTRMgrSoGstHdl, &gmute)) != eBTRMgrSOGstSuccess) {
         ...
         *mute = gmute;
         BTRMGRLOG_INFO("mute get %d success \n", gmute);
    }
...
BTRMgr_SO_GetEstimatedInABufSize (
    ...
    if (apstBtrMgrSoOutASettings->eBtrMgrOutAType == eBTRMgrATypeSBC) {
        ...
        lui16OutMtu         = floor((apstBtrMgrSoOutASettings->i32BtrMgrDevMtu - PACKET_OVERHEAD_SIZE)/lui16OutFrameLen) * lui16OutFrameLen;
    }
    ...
    lfOutMtuTimemSec = (lui16OutMtu * 1024.0) / lui32OutByteRate;
...
BTRMgr_SO_Start (
    ...
    unsigned short  lui16SbcFrameLen    = 0;
    ...
    if (apstBtrMgrSoOutASettings->eBtrMgrOutAType == eBTRMgrATypeSBC) {
        ...
        lui16SbcFrameLen   = pstBtrMgrSoOutSbcInfo->ui16SbcFrameLen;
    }
    ...
    if ((leBtrMgrSoGstRet = BTRMgr_SO_GstStart( pstBtrMgrSoHdl->hBTRMgrSoGstHdl,
                                                ...
                                                apstBtrMgrSoOutASettings->i32BtrMgrDevMtu,
                                                apstBtrMgrSoOutASettings->ui32BtrMgrDevDelay
                                                )) != eBTRMgrSOGstSuccess) {
        BTRMGRLOG_ERROR("Return Status = %d\n", leBtrMgrSoGstRet);
        leBtrMgrSoRet = eBTRMgrFailure;
    }
...
btrMgr_SO_GstStatusCb (
    ...
    if (pstBtrMgrSoHdl) {
        ...
        if ((pstBtrMgrSoHdl->fpcBSoStatus) && (bTriggerCb == TRUE)) {
            ...
            MEMCPY_S(&lstBtrMgrSoMediaStatus,sizeof(lstBtrMgrSoMediaStatus), &(pstBtrMgrSoHdl->lstBtrMgrSoStatus), sizeof(stBTRMgrMediaStatus));
