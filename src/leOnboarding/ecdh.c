  ...
  int* clear_text_length)
{
  ...
  MEMCPY_S(p,decrypted.length + 1, decrypted.data, decrypted.length);
...
baFromBase64(byteArray_t* decoded_data, char const* encoded_data)
{
  ...
  if(!b64) {
    BTRMGRLOG_ERROR("BIO_new failed ..\n");
    return 0;
  }
  buff = BIO_new_mem_buf(encoded_data, encoded_length);
  if(!buff) {
    BIO_free(b64);
    BTRMGRLOG_ERROR("BIO_new_mem_buf failed ..\n");
    return 0;
  }
  ...
  MEMSET_S(decoded_data->data, encoded_length,  0, encoded_length);
