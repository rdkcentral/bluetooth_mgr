...
#ifdef UNIT_TEST
static const tBTRMgrAcType BTRMGR_AC_TYPE_PRIMARY  = "primary";
static const tBTRMgrAcType BTRMGR_AC_TYPE_AUXILIARY = "auxiliary";
#else
//this is wrong should be replaced with unit_test code
 ...
 */
eBTRMgrRet BTRMgr_AC_Init (tBTRMgrAcHdl* phBTRMgrAcHdl, tBTRMgrAcType api8BTRMgrAcType, unsigned char ui8DebugMode);
...
/**
 * @brief This API initializes the bluetooth manager audio capture interface to read from a particular path.
 *
 * @param[in] phBTRMgrAcHdl Handle to the bluetooth manager audio capture interface.
 *
 * @return Returns the status of the operation.
 * @retval eBTRMgrSuccess on success, appropriate error code otherwise.
 */
eBTRMgrRet BTRMgr_AC_TestInit(tBTRMgrAcHdl* phBTRMgrAcHdl, unsigned char ui8DebugMode, const char* pcAudioSocket);


/**
 * @brief This API starts the bluetooth manager audio capture interface when configured to read from a particular path.
 *
 * @param[in] phBTRMgrAcHdl Handle to the bluetooth manager audio capture interface.
 *
 * @return Returns the status of the operation.
 * @retval eBTRMgrSuccess on success, appropriate error code otherwise.
 */
eBTRMgrRet BTRMgr_AC_TestStart(tBTRMgrAcHdl hBTRMgrAcHdl, stBTRMgrOutASettings* apstBtrMgrAcOutASettings, fPtr_BTRMgr_AC_DataReadyCb afpcBBtrMgrAcDataReady, fPtr_BTRMgr_AC_StatusCb afpcBBtrMgrAcStatus, void* apvUserData);

/**
 * @brief This API stops the bluetooth manager audio capture interface to read from a particular path.
 *
 * @param[in] phBTRMgrAcHdl Handle to the bluetooth manager audio capture interface.
 *
 * @return Returns the status of the operation.
 * @retval eBTRMgrSuccess on success, appropriate error code otherwise.
 */
eBTRMgrRet BTRMgr_AC_TestStop(tBTRMgrAcHdl hBTRMgrAcHdl);


/**
 * @brief This API deInits the bluetooth manager audio capture interface when configured to read from a particular path and frees all resources.
 *
 * @param[in] phBTRMgrAcHdl Handle to the bluetooth manager audio capture interface.
 *
 * @return Returns the status of the operation.
 * @retval eBTRMgrSuccess on success, appropriate error code otherwise.
 */
eBTRMgrRet BTRMgr_AC_TestDeInit(tBTRMgrAcHdl hBTRMgrAcHdl);
