...
getEncodedSBCFile (
    ...
    if (fgets(sbcEncodedFile, sizeof(sbcEncodedFile), stdin)) {
    }
    if (getchar() != EOF) {
    }//suck up a newline?
...
sendSBCFileOverBT (
    ...
    while (bytesLeft) {
        ...
        // write bluetooth
        if (timeout > 0) {
            if (fread (encoded_buf, bytesToSend, 1, sbcFilePtr) != 1) {
                fprintf (stderr, "reading on encoded_buf failed..\n");
            }
...
main (
    ...
    MEMSET_S(&stAppData, sizeof(stAppData), 0, sizeof(stAppData));
    ...
    if (enBTRCoreSuccess != BTRCore_GetVersionInfo(lhBTRCore, lpcBtVersion)) {
        printf ("BTR Bluetooth Version: FAILED\n");
    }
    printf("BTR Bluetooth Version: %s\n", lpcBtVersion);
    ...
    do {
        ...
        case 31:
            ...
            if(BTRCore_RegisterAgent(lhBTRCore, 1) != enBTRCoreSuccess)// 2nd arg controls the mode, 0 = NoInputNoOutput, 1 = DisplayYesNo
            {
                printf("Failed to Activate Agent\n");
            }
            break;
        case 32:
        ...
        case 45:
            {
                ...
                if (fgets (leUuidString, sizeof(leUuidString), stdin)) {
                }
        ...
        case 46:
            {
                ...
                if (aenBTRCoreLeOp == enBTRCoreLeOpGWriteValue) { 
                    ...
                    if (fgets(leWriteArg, sizeof(leWriteArg), stdin)) {
                    }
                }
...
extractWavHeader (
    ...
    if (pstAudioWaveHeader->ulWaveHeaderLength32Bits == 40 && pstAudioWaveHeader->usWaveHeaderFmt16Bits == 0xfffe) {
        ...
        if (fread(&pstAudioWaveHeader->ucFormatArr16x8Bits, 16, 1, fpInAudioFile) != 1) {
            fseek( fpInAudioFile, 0, SEEK_SET);
            return -1;
        }
    }   
...
doDataCapture (
    ...
    while (inFileBytesLeft) {
        ...
        if (fread (inDataBuf, inBytesToEncode, 1, inFileFp) != 1) {
            fprintf(stderr, "BTRMgr_SO_SendBuffer - fread - FAILED\n");
        }
...
streamOutTestMainAlternate (
    ...
    lstBtrMgrSoOutASettings.eBtrMgrOutAType   = eBTRMgrATypeUnknown;
#if 0
    struct timeval tv;
...
#else
    MEMSET_S(&lstAudioWavHeader, sizeof(lstAudioWavHeader), 0, sizeof(lstAudioWavHeader));
    ...
    while (inFileBytesLeft) {
        ...
        fread (inDataBuf, inBytesToEncode, 1, inFileFp);
...
streamOutLiveTestMainAlternateStart (
    ...
    if (eBTRMgrSuccess != BTRMgr_AC_Init(&(pstAppData->hBTRMgrAcHdl), NULL, 0)) {
...
streamInLiveTestMainAlternateStart (
    ...
    fprintf(stderr, "STREAMING IN NOT SUPPORTED");
    return -1;
#else
    int     inBytesToEncode= IN_BUF_SIZE;
